...
public class CarSalesExample {
   ...
   * 
   * @param saleCount
   *          the number of random sales that should be generated
   * @param seed
   *          the seed value for the random generator. For testing always use
  ...
  public static List<Sale<?>> generateSamples(int saleCount, long seed) {
    ...
    ArrayList<Sale<?>> sales = new ArrayList<Sale<?>>(20);
    ...
    for (int i = 0; i < saleCount; i++) {
      ...
      Sale<Car> sale = new Sale<Car>(randomCar, randomPrice, randomDate);
      sales.add(sale);
    }
    return sales;
  }
  ...
  public static LocalDate[] sampleRandomDates(long seed,
    ...
    ArrayList<Sale<?>> sorted = new ArrayList<Sale<?>>(sales);
    Collections.sort(sorted, new Comparator<Sale<?>>() {
      @Override
